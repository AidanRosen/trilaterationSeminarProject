...
class dbClass:
    ...
    def sendToDF(self, newRow):
        if self.check_conn():
            try:
                ...
                #conn = mysql.connection(host, self.port, autocommit=True)
                cursor = self.db.cursor()
                ...
                #db_connection.execute('INSERT INTO cse191.analytics_rimac_data (floor_name, x, y, ts_5mins) VALUES (' + ','.join('?' * len(newRow)) + ')' , newRow)
                #cursor.execute("INSERT INTO cse191.analytics_rimac_data VALUES(?, ?, ?, ?)", newRow)
                cursor.execute("INSERT INTO cse191.analytics_rimac_data VALUES ('{}', ' {}', '{}', '{}')".format(newRow[0], newRow[1], newRow[2], newRow[3]))
                self.db.commit()
                #cursor.execute('INSERT INTO cse191.analytics_rimac_data (floor_name, x, y, ts_5mins) VALUES (' + ','.join('?' * len(newRow)) + ')' , newRow)
    ...
    def getDF(self, time):
        if self.check_conn():
            ...
            new_datetime = start_time + timedelta(seconds=20)
    ...
    "3C:71:BF:64:26:74": 21, #X
            ...
            try:
                ...
                if results:
                    ...
                    rowarray = ["PLACEHOLDER", -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, time]
                    for i in range(0, len(results) - 1): 
                        ...
                        if results[i][3] != cur_device:
                            ...
                            rowarray = [results[i][3], -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, -90, time]
                            index = hashmap.get(results[i][1])
                            rowarray[index] = results[i][2]
